...
class ViMRCDatasetsForPhoBERTNoHapReflection(ViMRCDatasetsForPhoBERT):
    ...
    def postprocess_qa_predictions(
        ...
        for example_index, example in enumerate(tqdm(examples)):
            # Those are the indices of the features associated to the current example.
            feature_indices = features_per_example[example_index]
            features_per_example_ = collections.defaultdict(list)
            print("===================")
            print(features_per_example_)
            ...
            feature_index = sorted(np.array(feature_index_with_best_score).tolist(), key=lambda x: x[1], reverse=True)[0][0]
            prelim_predictions = []
            min_null_prediction = None
            # Looping through all the features associated to the current example.
            # for feature_index in feature_indices:
            ...
            # Compute the softmax of all scores (we do it with numpy to stay independent from torch/tf in this file, using
            # the LogSumExp trick).
            scores_ = np.array([pred.pop("score") for pred in predictions])
